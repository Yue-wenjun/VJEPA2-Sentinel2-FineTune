...
class MultiSeqWrapper(nn.Module):
    ...
    def forward(self, x, masks=None, gram_mode=False, training_mode=False, doys=None):
        """
        ...
        if masks is None:
            ...
            for x_fpc in x:
                if gram_mode:
                    ...
                    outputs.append(self.backbone(x_fpc, training=training_mode, doys=doys))
            return outputs

        outs = [[] for _ in x]
        for i, (x_fpc, m_fpc) in enumerate(zip(x, masks)):
            for m in m_fpc:
                outs[i] += [self.backbone(x_fpc, masks=m, training=training_mode, doys=doys)]
        return outs
